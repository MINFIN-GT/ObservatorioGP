...
public class UnidadEjecutoraDAO {
	...
	public static ArrayList<UnidadEjecutora> getUnidadesEjecutoras(Integer val_resultado, String tipo_resultaldo, Integer entidad){
		ArrayList<UnidadEjecutora> ret = new ArrayList<UnidadEjecutora>();
		String query = "";
		try{
			if(CMemsql.connect()){
				query = String.join(" ", "SELECT ejercicio,", 
						"       entidad, unidad_ejecutora, unidad_ejecutora_nombre,", 
				...
				UnidadEjecutora temp = null;
				while(rs.next()){
					if(programa_actual != rs.getInt("unidad_ejecutora")){
						if(temp != null)
							ret.add(temp);
						temp = (new UnidadEjecutoraDAO()).new UnidadEjecutora();
						temp.id = rs.getInt("unidad_ejecutora");
						temp.nombre_unidad_ejecutora = rs.getString("unidad_ejecutora_nombre");
